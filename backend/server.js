...
- Adapt the coping strategy to the specific exam (NEET student gets biology memory tips, UPSC student gets different advice)
...
{
  "pattern": "1 sentence describing the emotional pattern you detected (e.g. 'You feel most anxious on Sunday evenings before the week starts')",
...
app.post('/api/analyze', async (req, res) => {
  ...
  const safeJournal = sanitizeString(journalEntry);

  const userText = JSON.stringify({
    name: safeName,
    examType,
    moodScore: Number(moodScore),
    journalEntry: safeJournal,
